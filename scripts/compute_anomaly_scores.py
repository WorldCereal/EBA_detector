...
_ensure_worldcereal_importable = _ensure_packages_importable
...
# ===========================================================================
# Optional: recompute h3_l3_cell from lat/lon in the embeddings cache
# ===========================================================================


def recompute_h3_l3_cells_in_cache(
    embeddings_db_path: Path,
    *,
    batch_size: int = 200_000,
) -> int:
    """Recompute ``h3_l3_cell`` from ``lat``/``lon`` for every row in the cache.

    Triggered by ``--compute h3_levels``.  Useful when the cached H3 cell IDs
    are stale, missing, or were computed with a different convention and need
    to be regenerated before running outlier scoring.

    Returns the number of rows updated.
    """
    import h3  # local import to avoid hard dependency at module load

    if not embeddings_db_path.exists():
        raise FileNotFoundError(
            f"Embeddings DB not found at {embeddings_db_path}; "
            "run the embeddings pipeline first."
        )

    logger.info(f"[compute h3_levels] Recomputing h3_l3_cell from lat/lon in {embeddings_db_path}")
    ...
    try:
        ...
        if not total:
            logger.warning("[compute h3_levels] No rows with lat/lon found — nothing to do.")
            return 0
        logger.info(f"[compute h3_levels] {total:,} rows have lat/lon")

        offset = 0
        updated = 0
        while True:
            rows = con.execute(
                """SELECT sample_id, model_hash, lat, lon
                   FROM embeddings_cache
                   WHERE lat IS NOT NULL AND lon IS NOT NULL
                   ORDER BY sample_id, model_hash
                   LIMIT ? OFFSET ?""",
                [batch_size, offset],
            ).fetchdf()
            if rows.empty:
            ...
            rows["h3_l3_cell"] = [
                h3.latlng_to_cell(float(lat), float(lon), 3)
                for lat, lon in zip(rows["lat"].to_numpy(), rows["lon"].to_numpy())
            ]
            con.register("h3_updates", rows[["sample_id", "model_hash", "h3_l3_cell"]])
            con.execute(
                """UPDATE embeddings_cache AS e
                   SET h3_l3_cell = u.h3_l3_cell
                   FROM h3_updates AS u
                   WHERE e.sample_id = u.sample_id
                     AND e.model_hash = u.model_hash"""
            )
            con.unregister("h3_updates")
            updated += len(rows)
            logger.info(f"[compute h3_levels] {updated:,}/{total:,} rows updated")
            offset += batch_size
        con.execute("CHECKPOINT")
        logger.info(f"[compute h3_levels] Done — {updated:,} rows updated.")
        return updated
    finally:
        con.close()
...
def run_single_domain_scoring(
    ...
    **update mode**: call with ``embeddings_df=(df, embed_cols)`` — a pre-loaded
    ...
    if output_samples_path and Path(output_samples_path).exists() and not overwrite:
        existing_cols = set(pq.ParquetFile(str(output_samples_path)).schema.names)
        required = {"ref_id", "sample_id", config.label_domain,
                    "confidence_nonoutlier", "anomaly_flag"}
        if required.issubset(existing_cols):
            can_reuse = True
        else:
            missing = required - existing_cols
            logger.warning(
                f"existing {config.label_domain} output is incompatible "
                f"(missing {sorted(missing)}); re-running scoring instead. "
                f"File: {output_samples_path}"
            )
...
def compute_anomaly_scores(
    *,
    ...
    The ``compute`` argument is an optional list of one-off recomputations to
    perform before scoring.  Currently supported values:

    - ``"h3_levels"``: recompute ``h3_l3_cell`` from ``lat``/``lon`` for every
      row in the embeddings cache (overwriting any existing values).
    """
    ...
    # ------------------------------------------------------------------
    # Optional one-off recomputations (--compute ...)
    # ------------------------------------------------------------------
    if compute and "h3_levels" in compute:
        recompute_h3_l3_cells_in_cache(effective_db)
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    lc10.add_argument(
        "--lc10-class-mapping", type=str, default="LANDCOVER10",
        help=(
            "Key in the class mappings dict used for the 'LC10' domain "
            "(e.g. 'LANDCOVER10'). Also used as the label-column name."
        ),
    )
    ...
    cty.add_argument(
        "--cty24-class-mapping", type=str, default="CROPTYPE24",
        help=(
            "Key in the class mappings dict used for the 'CTY24' domain "
            "(e.g. 'CROPTYPE24', 'CROPTYPE_Mozambique'). Also used as the "
            "label-column name."
        ),
    )
    ...
    p.add_argument(
        ...
        default=None,
        help=(
            "One-off recomputations to perform before scoring. "
            "'h3_levels' recomputes h3_l3_cell from lat/lon in the embeddings "
            "cache (overwriting any existing values)."
        ),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    lc10_config = AnomalyRunConfig(
        ...
        class_mappings_name=args.lc10_class_mapping,
    ...
    cty24_config = AnomalyRunConfig(
        label_domain=args.cty24_class_mapping,
        class_mappings_name=args.cty24_class_mapping,
